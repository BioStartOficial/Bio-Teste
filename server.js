...
console.log(
  "Backend Init: FIREBASE_PROJECT_ID carregado:",
  firebaseConfig.projectId ? "Sim" : "Não"
); // DEBUG FIREBASE
...
app.post("/auth/register-firestore", async (req, res) => {
  ...
  try {
    ...
    if (!querySnapshot.empty) {
      return res
        .status(409)
        .json({ success: false, error: "Este email já está registado." });
    }
...
app.post("/auth/login-firestore", async (req, res) => {
  ...
  if (!email || !password) {
    return res
      .status(400)
      .json({ success: false, error: "Por favor, insira o email e a senha." });
  }
  ...
  try {
    ...
    if (querySnapshot.empty) {
      return res
        .status(401)
        .json({ success: false, error: "Email ou senha incorretos." });
    }
    ...
    if (passwordMatch) {
      res
        .status(200)
        .json({ success: true, user: userData, recordId: userDoc.id });
    } else {
      res
        .status(401)
        .json({ success: false, error: "Email ou senha incorretos." });
...
app.post("/registro", (req, res) =>
  res
    .status(405)
    .json({ success: false, error: "Use /auth/register-firestore" })
);
app.post("/login", (req, res) =>
  res.status(405).json({ success: false, error: "Use /auth/login-firestore" })
);
...
const getFirestoreCollectionName = (tableName) => {
  switch (tableName) {
    ...
    case "Checklists":
      return "checklists";
    default:
      return tableName.toLowerCase().replace(/\s/g, "_");
  }
};
...
const getFirestoreContent = async (res, tableName, fieldMapping) => {
  try {
    ...
    console.log(
      `DEBUG GET FIRESTORE: Chamando Firestore para coleção: ${getFirestoreCollectionName(
        tableName
      )}`
    );
...
const postToFirestore = async (res, tableName, fieldsToPost) => {
  try {
    const collectionRef = collection(db, getFirestoreCollectionName(tableName));
    console.log(
      `DEBUG POST FIRESTORE: Adicionando documento à coleção: ${getFirestoreCollectionName(
        tableName
      )}`
    );
...
const patchFirestoreContent = async (res, tableName, id, fieldsToUpdate) => {
  ...
  try {
    ...
    console.log(
      `DEBUG PATCH FIRESTORE: Atualizando documento ${id} na coleção: ${getFirestoreCollectionName(
        tableName
      )}`
    );
...
const deleteFirestoreRecord = async (tableName, id, res) => {
  try {
    const docRef = doc(db, getFirestoreCollectionName(tableName), id);
    console.log(
      `DEBUG DELETE FIRESTORE: Deletando documento ${id} da coleção: ${getFirestoreCollectionName(
        tableName
      )}`
    );
...
app.get("/content/educational-texts", (req, res) =>
  getFirestoreContent(res, "Conteudo Educativo", (record) => ({
    id: record.id,
...
app.get("/content/quizzes", (req, res) =>
  getFirestoreContent(res, "Quizzes", (record) => {
    let questions = [];
    try {
      if (record.fields.Perguntas) {
        // No Firestore, 'Perguntas' será uma string JSON
      ...
      console.error("Erro JSON no quiz (Firestore):", record.id, e);
    }
...
app.get("/content/checklists", (req, res) =>
  getFirestoreContent(res, "Checklists", (record) => {
    let items = [];
    try {
      if (record.fields.items) {
        // No Firestore, 'items' será uma string JSON
        const parsedItems = JSON.parse(record.fields.items);
        if (Array.isArray(parsedItems)) {
          items = parsedItems.map((item) => ({
            text: item.text,
            completed: item.completed || false,
          })); // Adapta para o formato esperado
        }
      }
    } catch (e) {
      console.error("Erro JSON no checklist (Firestore):", record.id, e);
    }
...
app.delete("/content/quizzes/:id", (req, res) =>
  deleteFirestoreRecord("Quizzes", req.params.id, res)
);
app.delete("/content/educational-texts/:id", (req, res) =>
  deleteFirestoreRecord("Conteudo Educativo", req.params.id, res)
);
app.delete("/content/checklists/:id", (req, res) =>
  deleteFirestoreRecord("Checklists", req.params.id, res)
);
...
app.post("/forum/post-question", async (req, res) => {
  ...
  if (!question || !author) {
    return res
      .status(400)
      .json({ success: false, error: "Questão e autor são obrigatórios." });
  }
  try {
...
app.post("/user/quiz-score", async (req, res) => {
  ...
  if (!userId || score === undefined || totalQuestions === undefined) {
    return res
      .status(400)
      .json({ success: false, error: "Dados do quiz incompletos." });
  }
  try {
    const userDocRef = doc(db, "users", userId);
    await updateDoc(userDocRef, {
